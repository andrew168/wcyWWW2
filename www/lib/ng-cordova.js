...
angular.module('ngCordova.plugins.socialSharing', [])
  ...
  .factory('$cordovaSocialSharing', ['$q', '$window', function ($q, $window) {
    ...
    return {
      ...
      shareViaFacebookWithPasteMessageHint: function (message, file, link, pastPrompt) {
          var q = $q.defer();
          $window.plugins.socialsharing.shareViaFacebookWithPasteMessageHint(message, file, link, pastPrompt,
              function (success) {
                  q.resolve(success);
              },
              function (error) {
  ...
  .factory('$cordovaSQLite', ['$q', '$window', function ($q, $window) {
    ...
    return {
      openDB: function (dbName, background) {
        ...
        if (!$window.sqlitePlugin) { // support PC for debug
            return window.openDatabase(dbName, "1.0", "Demo", background);
        }
